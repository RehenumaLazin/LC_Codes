...
def load_model(model_path, device='cuda'):
    model = UNet(in_channels=4, out_channels=1, init_features=32)
...
def k_fold_cross_validation_based_on_events(events,events_file, dataset, num_epochs=int, batch_size=128, model_dir = str, checkpoint_path=str, device='cuda', device_ids=device_ids):
    ...
    model = UNet(in_channels=4, out_channels=1, init_features=32).to(device)
    model = nn.DataParallel(model).to(device)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_prec_dem_LC"
    ...
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_LC"
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_prec_dem_LC/unet_model_fold_2.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
