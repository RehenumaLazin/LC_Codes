...
if sys.argv[1] == "train":
    ...
    for e, EVENT_STR in enumerate(EVENT_STRS):
        ...
        events = pd.read_csv(event_file, header=None).to_numpy()
        model_dir = f"/p/vast1/lazin1/UNet_trains/Event_wise_with_SM_streamflow_no_waterbody/{EVENT_STR}"
        ...
        events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined_{EVENT_STR}_No_Waterbody.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined.csv"  #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv'
        ...
        for tile_name in tile_names:
            tile_str = tile_name.split("crop")[0][:-1] 
            DEM.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_DEM_No_Waterbody/{tile_str}/dem_{tile_name}")
            LC.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_LC_No_Waterbody/{tile_str}/LC_{tile_name}")
            day1_prec.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/1D_prec_No_Waterbody/{tile_str}/1day_prec_{tile_name}")
            day5_prec.append( f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/5D_prec_No_Waterbody/{tile_str}/5day_prec_{tile_name}")
            streamflow.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/streamflow_No_Waterbody/{tile_str}/streamflow_{tile_name}")
            SM.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_SM_No_Waterbody/{tile_str}/SM_{tile_name}")
