...
class NSELoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        denominator = torch.sum((y_true - torch.mean(y_true)) ** 2) + self.eps
        nse = 1 - numerator / denominator
        loss = 1 - nse  # Because we want to minimize the loss
        return loss
...
def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs=int, start_epoch=int, fold=int, checkpoint_path=str, device='cuda'):
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                ...
                with autocast():
                    ...
                    # loss = criterion(outputs_denorm, target)
                    loss = criterion(outputs, targets)
                ...
                nse_score += NSE(outputs, targets)



        val_loss /= len(val_loader.dataset)
        # iou_score /= len(val_loader)
        nse_score /= len(val_loader)
        val_losses.append(val_loss)
        # miou.append(iou_score)
        nse.append(nse_score)
        # print(f"Validation Loss: {val_loss:.4f}, Validation mIoU: {iou_score:.4f}")
        print(f"Validation Loss: {val_loss:.4f}, Validation NSE: {nse_score:.4f}")
    ...
    return model, train_losses, val_losses, nse
...
def k_fold_cross_validation_based_on_events(model, events,events_file, dataset, num_epochs=int, batch_size=128, checkpoint_path=str, device='cuda', device_ids=device_ids):
    ...
    for fold,  event in enumerate(events, start=start_fold):
        ...
        trained_model, train_losses, val_losses, nse = train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs,start_epoch, fold, checkpoint_path, device)
        ...
        np.save(os.path.join(model_dir, f"val_nse_{fold + 1}.npy"),nse)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2_org_code_output_not_nprmalized_NSE"
    ...
    for e, event in enumerate(events, start=1):
        ...
        k_fold_cross_validation_based_on_events(model, events, events_file, dataset, num_epochs=100, batch_size=128, checkpoint_path=checkpoint_path, device=device, device_ids=device_ids)
    ...
    output_geotiff_dir = f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_org_output_not_normalized"
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_org_code_output_not_nprmalized/unet_model_fold_7.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
