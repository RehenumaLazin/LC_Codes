...
for f in files:        
    ...
    if os.path.exists(temp_raster_path): continue
    ...
    if not os.path.exists(temp_raster_path):
        ...
        start_date_dt = datetime.fromtimestamp(datetime.strptime(end_date_str, "%Y%m%d%H").timestamp()) - timedelta(hours=1)
        end_date_dt = datetime.strptime(end_date_str , "%Y%m%d%H")
        start_date = start_date_dt.strftime( "%Y-%m-%d %H")
        end_date = end_date_dt.strftime( "%Y-%m-%d %H")
        print(start_date, end_date)

        
        start_ym = start_date.split("-")[0] + "_" + start_date.split("-")[1]
        end_ym = end_date.split("-")[0] +'_'+ end_date.split("-")[1] 
