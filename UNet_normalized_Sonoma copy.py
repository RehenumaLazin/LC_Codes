...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nan_mask_dem = np.isnan(input_array_normalized[0,:,:])
        print('nan_mask_dem',np.sum(nan_mask_dem))
        ...
        nan_mask_day1_prec = np.isnan(input_array_normalized[1,:,:])
        print('nan_mask_day1_prec',np.sum(nan_mask_day1_prec))
        
        input_array_normalized[2,:,:] = (inputs[2,:,:] - np.mean(day5_prec_mean)) / np.mean(day5_prec_std)
        nan_mask_day5_prec = np.isnan(input_array_normalized[2,:,:])
        print('nan_mask_day5_prec',np.sum(nan_mask_day5_prec))
        
        # input_array_normalized[4,:,:] = (inputs[4,:,:] - np.mean(day30_prec_mean)) / np.mean(day30_prec_std)
        input_array_normalized[3,:,:] = (inputs[3,:,:] - np.mean(flow_mean)) / np.mean(flow_std)
        nan_mask_flow = np.isnan(input_array_normalized[3,:,:])
        print('nan_mask_flow',np.sum(nan_mask_flow))
        
        array = inputs[4,:,:]
        array[array == -9999] = 1  # Permanent waterbody is -9999
        input_array_normalized[4,:,:] = (array - np.mean(SM_mean)) / np.mean(SM_std)
        
        nan_mask_SM = np.isnan(input_array_normalized[4,:,:])
        print('nan_mask_SM',np.sum(nan_mask_SM))
...
def load_unet_model(model_class, model_path, in_channels=5, out_channels=1, initial_filters=32, device=None):
    """
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                inputs, targets = inputs.to(device), targets.to(device)
                # print(torch.isnan(inputs).any())         # Should be False
...
def k_fold_cross_validation_based_on_events(model, events,events_file, dataset, num_epochs=int, batch_size=128, checkpoint_path=str, device='cuda', device_ids=device_ids):
    ...
    for fold,  event in enumerate(events, start=start_fold):
        ...
        trained_model, train_losses, val_losses, mse = train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs,start_epoch, fold, checkpoint_path, device)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2"
    os.makedirs(model_dir,exist_ok=True)
    
    
    checkpoint_path = os.path.join(model_dir,"model_checkpoint.pth")
    for e, event in enumerate(events, start=1):
        model_save_path = os.path.join(model_dir, f"unet_model_fold_{e}.pt") #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/normalized_UNet/unet_model_fold_{fold + 1}.pt"
        
        if e > 1 and os.path.exists(model_save_path):
            ...
            model = UNet(in_channels=5, out_channels=1, init_features=32).to(device)
            model = nn.DataParallel(model).to(device)
        ...
        #     # model = nn.DataParallel(model).to(device)
        #     model_dir = f"/p/vast1/lazin1/UNet_trains/Event_wise_with_SM_streamflow/{EVENT_STR}"
        ...
        for tile_name in tile_names:
            ...
            print(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_DEM/Sonoma/case{tile_str}")
        ...
        k_fold_cross_validation_based_on_events(model, events, events_file, dataset, num_epochs=30, batch_size=128, checkpoint_path=checkpoint_path, device=device, device_ids=device_ids)
    ...
    for tile_name in tile_names:
        ...
        streamflow.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/streamflow/{tile_str}/streamflow__{tile_name}")
