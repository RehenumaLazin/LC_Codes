...
events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/Sonoma_shapefile/Sonoma_events.csv" #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv' #combined.csv'
...
for idx, (s,e)  in enumerate(zip(start_dates, end_dates)): #events = [raster_path.split("/")[-1][:-4] 
    print(s,e)
    if idx not in [3, ]: continue
...
# Input files
    event_dir = f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/WM/Tile_Sonoma/event{idx+1}"  # Folder to save the cropped DEMs  #Mississippi_20190617_5E5F_cropped_dems_non_flood
    ...
    if os.path.exists(temp_raster_dir):
        ...
        files = [f"{temp_raster_dir}/depth-1-{i * 1440}.tif" for i in range(1, days + 1)] # Get the list of files at 24 th hour interval (24*60)
        for d, file in enumerate(files):
            ...
            for reference_raster_file in geo_files:
                ...
                output_geotiff = f"{event_dir}/event{idx+1}_{next_day}_{f}"
