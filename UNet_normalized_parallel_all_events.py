...
class FloodDataset(Dataset):
    def __init__(self, input_geotiff_paths, label_geotiff_path=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        input_array_normalized[5,:,:] = (inputs[5,:,:] - np.mean(SM_mean)) / np.mean(SM_std)
        ...
        if self.label_geotiff_path:
            ...
            return torch.tensor(input_array_normalized, dtype=torch.float32), torch.tensor(label, dtype=torch.float32).unsqueeze(0) , transform, crs
        return torch.tensor(input_array_normalized, dtype=torch.float32), transform, crs
...
def custom_collate_fn(batch):
    """
    Custom collate function to handle non-tensor metadata like transform and crs.
    """
...
def train_model( model, criterion, optimizer,e , train_loader, val_loader, num_epochs=25, model_dir=str, fold=int, device='cuda'):
    ...
    for epoch in range(e, num_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, f"unet_model_fold_{fold}_{epoch}.pt")) #torch.save(model.state_dict(), str(model_path))
...
def k_fold_cross_validation_based_on_events( dataset, num_epochs=30, batch_size=64, device='cuda',device_ids=device_ids):
    
    pt_files = [os.path.join(model_dir, f) for f in os.listdir(model_dir) if f.endswith('.pt')]
    if not pt_files:
        e=0
        f =1
        # return None  # No .pt files found
    else:
        latest_file = max(pt_files, key=os.path.getmtime)
        e = int(latest_file.split("_")[-1][:-3]) + 1
        f = int(latest_file.split("fold")[-1].split("_")[1])
            
    # model_save_path = os.path.join(model_dir, f"unet_model_fold_{fold}_{epoch}.pt")
    if f ==1 and e==0:
    ...
    # elif f > 1 and e==0:
    #     #Load model
    #     model = load_model(os.path.join(model_dir, f"unet_model_fold_{f-1}_{num_epochs-1}.pt"), device='cuda')
    #     print('loaded', os.path.join(model_dir, f"unet_model_fold_{f-1}_{num_epochs-1}.pt"))
    ...
    #     print('loaded', os.path.join(model_dir, f"unet_model_fold_{f-1}_{num_epochs-1}.pt"))
        ...
        model = load_model(os.path.join(model_dir, f"unet_model_fold_{f}_{e-1}.pt"), device='cuda')
        print('loaded', os.path.join(model_dir, f"unet_model_fold_{f}_{e-1}.pt"))
    ...
    #     print(f"Fold {fold + 1}/{k}")
        ...
        #     print(f"Batch Inputs: {batch_inputs.shape}")
        #     print(f"Batch Labels: {batch_labels.shape}")
        val_loader = DataLoader(dataset, batch_size=batch_size, sampler=val_sampler)

        # # Initialize Model
        model_save_path = os.path.join(model_dir, f"unet_model_fold_{fold}.pt")
        # if fold ==1:
        ...
        # else:
        #     #Load model
        #     model = load_model(os.path.join(model_dir, f"unet_model_fold_{fold-1}.pt"), device='cuda')
        #     # model_save_path = os.path.join(model_dir, f"unet_model_fold_{fold}.pt")
            

        trained_model, train_losses, val_losses, miou = train_model(model, criterion, optimizer, e, train_loader, val_loader, num_epochs,model_dir, fold, device)
        ...
        # np.save(os.path.join(model_dir, f"train_val_losses_fold_{fold}.npy") , fold_losses)
        # # np.save(f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/normalized_UNet/val_miou_{fold + 1}.npy" , miou)
        # np.save(os.path.join(model_dir, f"val_miou_{fold}.npy"),miou)
...
model_dir = f"/p/vast1/lazin1/UNet_trains/All_events_shuffled"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
if sys.argv[1] == "train":
    ...
    events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined_reduced_events.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined.csv"  #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv'
    ...
    k_fold_cross_validation_based_on_events(dataset, num_epochs=10, batch_size=128, device=device, device_ids=device_ids)
...
#     k_fold_test_output_array(dataset, k=5,  batch_size=64, device='cuda',device_ids=device_ids)
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
