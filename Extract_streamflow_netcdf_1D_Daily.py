...
def extract_streamflow(target_idxs,target_gauges,start_year, end_year):
    ...
    for i, gauge in enumerate(target_gauges):
            ...
            for month in range(1, 13):
                for day in range(1, calendar.monthrange(year, month)[1] + 1):  # Handle invalid dates dynamically
                    try:
                        ...
                        dataset = xr.open_dataset(f"/p/lustre2/lazin1/NOAA_streamflow_WRF_Hydro/{year}/{date_str}.nc")
                        ...
                        date.append(f"{date_str}")
                        flow_cms.append(streamflow_values)
        ...
        flow_data.to_csv(f"{data_dir}/Timeseries/{gauge}_daily.csv", index=False)             
