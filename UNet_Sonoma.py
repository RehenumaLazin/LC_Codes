...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_array = np.empty(inputs.shape, dtype=np.float32)
        input_array[0,:,:] = inputs[0,:,:]# - np.mean(DEM_mean)) / np.mean(DEM_std)
        input_array[1,:,:] =  np.where(inputs[1,:,:] == 11, 1, 0)     
        input_array[2,:,:] = inputs[2,:,:]# - np.mean(day1_prec_mean)) / np.mean(day1_prec_std)
        input_array[3,:,:] = inputs[3,:,:]# - np.mean(day5_prec_mean)) / np.mean(day5_prec_std)
        # input_array_normalized[4,:,:] = (inputs[4,:,:] - np.mean(day30_prec_mean)) / np.mean(day30_prec_std)
        input_array[4,:,:] = inputs[4,:,:]# - np.mean(flow_mean)) / np.mean(flow_std)
        array = inputs[5,:,:]
        array[array == -9999] = 1  # Permanent waterbody is -9999
        input_array[5,:,:] = array # - np.mean(SM_mean)) / np.mean(SM_std)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2_all_not_normalized"
    ...
    k_fold_cross_validation_based_on_events(events, events_file, dataset, num_epochs=30, batch_size=128, model_dir=model_dir, checkpoint_path=checkpoint_path, device=device, device_ids=device_ids)
    ...
    events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/Sonoma_shapefile/Tile_Sonoma_event3.csv"#f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined_reduced_events_test.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined.csv"  #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv'
    ...
    os.makedirs(f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event3_all_not_normalized", exist_ok=True)
    for tile_name in tile_names:
        tile_str = tile_name.split("Sonoma")[-1]#tile_name.split("crop")[0][:-1] 
        event = tile_name.split("/")[-1].split("_")[0]
        date = tile_name.split("/")[-1].split("_")[1]
        ...
        label_geotiff_path.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/WM/Tile_Sonoma/{event}/{tile_name}") 
        output_geotiff_paths.append(f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event3/{tile_name}") 
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_all_not_normalized/unet_model_fold_3.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
