...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dem_norm = self._load_static_inputs(tile)
        ...
        # print('PRINT',dem_norm.shape, p1d_norm.shape, p5d_norm.shape)
        inputs = np.stack([dem_norm, p1d_norm, p5d_norm], axis=0).astype(np.float32)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    k_fold_cross_validation_based_on_events(events, events_file, dataset, prev_trained_days=1, num_epochs=50, batch_size=128, model_dir=model_dir, checkpoint_path=checkpoint_path, device=device, device_ids=device_ids)
    ...
    label_geotiff_path = []
    # output_geotiff_paths =[]
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_retrain_multiple/0909"
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_prec_dem_retrain/unet_model_fold_23.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
