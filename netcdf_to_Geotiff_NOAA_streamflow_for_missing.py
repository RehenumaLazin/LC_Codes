...
events_file = '/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/missing_streamflow.csv' #/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/missing_streamflow.csv
...
for idx, raster_path in enumerate(combined_df[0]): #events = [raster_path.split("/")[-1][:-4] 
    event = raster_path.split("/")[-1].split("crop")[0][11:-1]
    # print(event)
...
# TARGET_RASTER_DIR = f"/p/lustre1/lazin1/RAPID_Archive_Flood_Maps/Tile_No_Threshold_{EVENT_STR}" #'/p/lustre1/lazin1/RAPID_Archive_Flood_Maps/Tile_No_Threshold_Mississippi_20190617_5E5F_non_flood'  # Reference raster for extent and resolution
    ...
    output_geotiff_dir = f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/streamflow/{event}" #Harvey_D374'  #5E5F
    ...
    reference_raster_file = os.path.join(TARGET_RASTER_DIR, raster_path.split("/")[-1][11:])
    ...
    if not os.path.exists(temp_raster_path):
        ...
        with rasterio.open(
            ...
            dst.write(np.nan_to_num(grid_streamflow), 1)  # Fill NaNs with zeros or appropriate values
