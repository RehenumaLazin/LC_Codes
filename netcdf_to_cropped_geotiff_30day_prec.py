...
# NetCDF file with hourly precipitation
...
# # EVENT_STRS = ["Harvey_20170829_B8C4_non_flood", "Harvey_20170829_3220_non_flood"]
...
def sum_precipitation(precip_data, start_idx, end_idx):
    sum_precip = np.zeros(precip_data[0, :, :].shape)
    for i in range(start_idx,end_idx+1):
        
        # Sum the data across the specified time range
        sum_precip = np.add(sum_precip , precip_data[i, :, :])
...
for idx, raster_path in enumerate(combined_df[0]): #events = [raster_path.split("/")[-1][:-4] 
    ...
    output_geotiff_dir = f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/30D_prec/{event}" #30_day_AORC_Prec_Mississippi_20190617_5E5F_non_flood
    if os.path.exists(output_geotiff_dir):
        ...
        for reference_raster_file in reference_raster_files:
            ...
            output_geotiff = os.path.join(output_geotiff_dir, '30day_prec_' + reference_raster_file.split("/")[-1])
        
            precip_30day= []
            # print(reference_raster_file[1])
            reference_raster = reference_raster_file#.split("/")[-1] #reference_raster_file.split("/")[-1]
            ...
            if not os.path.exists(temp_raster_path):
                ...
                output_geotiff = os.path.join(output_geotiff_dir, '30day_prec_' + reference_raster_file.split("/")[-1]) #reference_raster_file[1])
                end_date_str = reference_raster_file.split("/")[-1].split("crop")[0][-51:-43] # reference_raster_file[2][:-7] ##MAY BE reference_raster_files[0].split("/")[-1].split("crop")[0][-51:-43]
                ...
                # print(day, calendar.monthrange(y,m)[1])
                ...
                # end_date_dt = datetime.fromtimestamp(datetime.strptime(end_date_str +" 23" , "%Y%m%d %H").timestamp())
                start_date_dt = datetime.fromtimestamp(datetime.strptime(end_date_str +" 23" , "%Y%m%d %H").timestamp()) - timedelta(days=30) + timedelta(hours=1)
                ...
                if start_date_dt.year == end_date_dt.year and start_date_dt.month == end_date_dt.month:
                    ...
                    precip_30day = sum_precipitation(precip_var, start_date_idx, end_date_idx)
                    # np.sum(precip_var[start_date_idx:end_date_idx+1, :, :], axis=0)
                    ...
                    # latitudes = nc_end.variables['latitude'][:]
                    # longitudes = nc_end.variables['longitude'][:]
                    ...
                    # Sum precipitation over the previous 24 hours (1 day)
                    precip_sum_start = sum_precipitation(precip_var_start, start_date_idx, precip_var_start.shape[0]-1)
                    # np.sum(precip_var_start[start_date_idx:-1, :, :], axis=0)
                    precip_sum_end = sum_precipitation(precip_var_end, 0, end_date_idx)
                    # np.sum(precip_var_start[0:end_date_idx+1, :, :], axis=0)
                    
                    precip_30day = np.sum([precip_sum_start, precip_sum_end], axis=0)
                    ...
                    # Sum precipitation over the previous 24 hours (1 day)
                    precip_sum_start = sum_precipitation(precip_var_start, start_date_idx, precip_var_start.shape[0]-1)
                    precip_sum_middle = sum_precipitation(precip_var_middle, 0, precip_var_middle.shape[0]-1)
                    # np.sum(precip_var_start[start_date_idx:-1, :, :], axis=0)
                    precip_sum_end = sum_precipitation(precip_var_end, 0, end_date_idx)
                    
                    precip_30day = np.sum([precip_sum_start, precip_sum_middle,precip_sum_end], axis=0)     
            ...
            print(f"30-day precipitation GeoTIFF saved at {output_geotiff}")
