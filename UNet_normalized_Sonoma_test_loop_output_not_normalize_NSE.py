...
def k_fold_cross_validation_based_on_events(events,events_file, dataset, num_epochs=int, batch_size=128, model_dir = str, checkpoint_path=str, device='cuda', device_ids=device_ids):
    ...
    for fold,  event in enumerate(events, start=start_fold):
        ...
        trained_model, train_losses, val_losses, nse = train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs,start_epoch, fold, checkpoint_path, device, scaler=scaler)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_NSE_retrain"
    ...
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_NSE"
