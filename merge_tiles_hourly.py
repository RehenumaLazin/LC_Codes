...
#/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_LC_Hourly_script_fold11/test


event= f"Sonoma_event2_prec_dem_LC_Hourly_script_fold11/" #f"Sonoma_event2_prec_dem_LC_Hourly" #f"Sonoma_event2_prec_dem_LC_Hourly_script"#f"Sonoma_event2_prec_dem_LC" #f"Sonoma_event2_output_not_normalized_retrain" #"Sonoma_event2_org_output_not_normalized"
...
event_dir=f"{output_geo_dir}/{event}/test"

sequence_length = 5
start_time = datetime.strptime("2019022705", "%Y%m%d%H")
...
for h in all_hours:
    ...
    event_str=f"{h.strftime('%Y%m%d%H')}_Sonoma"
    print(event_str)
