...
def test_unet_on_dataset(model, dataset, output_geotiff_paths, device):
    ...
    for i, data in enumerate(dataloader):
        ...
        with rasterio.open(
            ...
            "w",
            driver="GTiff",
            height=outputs_denorm.shape[0],
            width=outputs_denorm.shape[1],
            count=1,
            dtype="float32",
            crs=crs,
            transform=transform,
        ) as dst:
            dst.write(outputs_denorm, 1)
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"//p/vast1/lazin1/UNet_trains/Sonoma_event2_all_normalized"
    ...
    os.makedirs(f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event3_all_normalized", exist_ok=True)
    for tile_name in tile_names:
        tile_str = tile_name.split("Sonoma")[-1]#tile_name.split("crop")[0][:-1] 
        event = tile_name.split("/")[-1].split("_")[0]
        date = tile_name.split("/")[-1].split("_")[1]
        ...
        label_geotiff_path.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/WM/Tile_Sonoma/{event}/{tile_name}") 
        output_geotiff_paths.append(f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event3_all_normalized/{tile_name}") 
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_all_normalized/unet_model_fold_3.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
