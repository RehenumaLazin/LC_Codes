...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_array_normalized[2,:,:] = (inputs[2,:,:] - np.mean(H1_prec_mean)) / np.mean(H1_prec_std)
        ...
        for inputs, targets in train_loader:
            ...
            with autocast():
                try:
                    ...
                    print("ERROR in model forward pass:", str(e))
                    raise
...
# for e, EVENT_STR in enumerate(EVENT_STRS):
    ...
    model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_prec_dem_LC_HOURLY"
    ...
    events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/Sonoma_shapefile/Tile_Sonoma_hourly_event2.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined_{EVENT_STR}_No_Threshold.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined.csv"  #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv'
    ...
    for tile_name in tile_names:
        ...
        date = tile_name.split("/")[-1].split("_")[1][:-2]
        datetime= tile_name.split("/")[-1].split("_")[1]
        date_tile_name = tile_name.replace(datetime, date)
        ...
        H1_prec.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/1H_prec/Sonoma/{event}/1H_prec_{tile_name}")
        day1_prec.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/1D_prec/Sonoma/{event}/1day_prec_{date_tile_name}")

        day5_prec.append( f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/5D_prec/Sonoma/{event}/5day_prec_{date_tile_name}")
        streamflow.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/streamflow/Sonoma/{event}/streamflow_{date_tile_name}")
        SM.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_SM/Sonoma/{event}/SM_{date_tile_name}")
        
        label_geotiff_path.append(f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/WM/Tile_Sonoma_hourly/{event}/{tile_name}") 
    ...
    k_fold_cross_validation_based_on_events(events, events_file, dataset, num_epochs=10, batch_size=128, model_dir=model_dir, checkpoint_path=checkpoint_path, device=device, device_ids=device_ids)
    ...
    events_file = f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/Sonoma_shapefile/Tile_Sonoma_hourly_event2.csv"#f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined_reduced_events_test.csv" #f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/Codes/combined.csv"  #'/usr/workspace/lazin1/anaconda_dane/envs/RAPID/EVENTS/combined.csv'
    ...
    model_path = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_not_normalized_prec_dem_LC_HOURLY/unet_model_fold_3.pt" #f"/p/vast1/lazin1/UNet_trains/Mississippi_20190617_5E5F_non_flood_event_wise/unet_model_fold_5.pt" #"/p/vast1/lazin1/UNet_trains/All_events_shuffled/unet_model_fold_8_9.pt"  # Path to the trained model
