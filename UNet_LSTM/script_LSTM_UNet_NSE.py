...
class FloodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for h in seq_hours:
            ...
            #     output_geotiff_path = os.path.join(self.geo_dir, f"{h.strftime('%Y%m%d%H')}_{tile}")
        ...
        precip_tensor = torch.from_numpy(precip_seq).unsqueeze(1)
...
class UNetLSTM(nn.Module):
    def __init__(self, in_static_ch=2, in_seq_ch=1, seq_len=10, hidden_dim=512):
        ...
        self.lstm = nn.LSTM(input_size=128 * 8 * 8, hidden_size=hidden_dim, batch_first=True)
    ...
    def forward(self, static_input, seq_input):
        ...
        for t in range(T):
            ...
            x3_pooled = F.adaptive_avg_pool2d(x3, (8, 8)) # Or even (8, 8)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        #     seq = inputs[:, 2:].unsqueeze(2) 
            ...
            if loss is not None and torch.isfinite(loss):
                ...
                # print("Tracked optimizers by GradScaler:", scaler._per_optimizer_states.keys())
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()
...
#     # Check if DataParallel is needed
#     if torch.cuda.device_count() > 1:
#         model = nn.DataParallel(model)
#     model = model.to(device)

#     # Handle full checkpoint or raw state_dict
...
model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_prec_dem_LC_HOURLY_LSTM_UNET_NSE"
...
precip_dir = "/p/vast1/lazin1/UNet_inputs/Geotiff_var/1H_prec/Sonoma/event1"
flood_dir = "/p/vast1/lazin1/UNet_inputs/Geotiff_var/WM/Tile_Sonoma_hourly/event1"
...
if sys.argv[1] == "train":
    ...
    # print(dataset[0])
    kfold_train(dataset, k_folds=11, prev_trained_days=6, batch_size=8, model_dir=model_dir, num_epochs=2)
    ...
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_LC_Hourly_script_LSTM_UNet_NSE_fold11"
