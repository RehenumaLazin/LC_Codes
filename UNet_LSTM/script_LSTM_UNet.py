...
class UNetLSTM(nn.Module):
    ...
    def forward(self, static_input, seq_input):
        device = static_input.device  # this will be correct for each replica
        seq_input = seq_input.to(device)  # ensure both inputs are on the same device
...
def train(model, train_loader, val_loader, optimizer, criterion, checkpoint_path, scaler, fold_idx, start_epoch, num_epochs=10):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for static, seq, target in tqdm(train_loader, desc=f"Epoch {epoch}"):
            # print(static.shape)  # should be (B, 2, H, W)
            # print(seq.shape)     # should be (B, T, 1, H, W)
            # print(target.shape)   # should be (B, 1, H, W)
...
model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_prec_dem_LC_HOURLY_LSTM_UNET"
...
if sys.argv[1] == "train":
    ...
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_LC_Hourly_script_LSTM_UNet_fold11"
