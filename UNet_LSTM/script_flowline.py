...
class FloodDataset(Dataset):
    def __init__(self, event, dem_dir, lc_dir, flowline_dir, precip_dir, flood_dir, tile_csv, hours, seq_len,
                 DEM_mean, DEM_std, precip_mean, precip_std,
    ...
    def _load_static_inputs(self, tile):
        ...
        dem, _, _ = self._read_raster(dem_path)
        lc, _, _ = self._read_raster(lc_path)
        flowline, _, _ = self._read_raster(lc_path)
...
class UNetLSTM(nn.Module):
    def __init__(self, in_static_ch=3, in_seq_ch=1, seq_len=10, hidden_dim=64):
        super().__init__()
...
model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_event2_output_prec_dem_LC_flowline_HOURLY_script_"
...
if sys.argv[1] == "train":
    ...
    flowline_dir = f"/p/vast1/lazin1/UNet_inputs/Geotiff_var/cropped_flowline/Sonoma"
    ...
    dataset = FloodDataset(event, dem_dir, lc_dir, flowline_dir, precip_dir, flood_dir, tile_csv, all_hours, sequence_length,
                            DEM_mean, DEM_std, H1_prec_mean, H1_prec_std,
                            flood_mean=None, flood_std=None, output_geotiff_dir=None, return_metadata=False, mode='train')
    ...
    # print(dataset[0])
    kfold_train(dataset, k_folds=3, batch_size=16, prev_trained_days=5,  model_dir=model_dir, num_epochs=20)
...
#     dataset = StaticFloodDataset(all_data)
    ...
    model_path = f"{model_dir}/unet_model_fold_6.pt"
