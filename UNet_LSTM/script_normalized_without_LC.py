...
class FloodDataset(Dataset):
    def __init__(self, event, dem_dir, lc_dir, precip_dir, flood_dir, tile_csv, hours, seq_len,
                 DEM_mean, DEM_std, precip_mean, precip_std,
                 flood_min=None, flood_max=None, output_geotiff_dir=None, return_metadata=False, mode='train'):
        self.event = event
        self.dem_dir = dem_dir
        self.lc_dir = lc_dir
        self.precip_dir = precip_dir
        ...
        self.precip_mean = precip_mean
        self.precip_std = precip_std
        self.flood_min = flood_min
        self.flood_max = flood_max
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if self.flood_min is not None and self.flood_max is not None and self.mode == 'train':
                label = (label - self.flood_min) / self.flood_max
...
class UNetLSTM(nn.Module):
    def __init__(self, in_static_ch=1, in_seq_ch=1, seq_len=10, hidden_dim=64):
        super().__init__()
    ...
    #     seq_input = seq_input.to(static_input.device)
    #     print(f"[DEBUG] static_input device: {static_input.device}")
    #     print(f"[DEBUG] seq_input device: {seq_input.device}")
    #     print(f"[DEBUG] model parameter device: {next(self.parameters()).device}")
    ...
    #     x_seq = x_seq.mean(dim=[3, 4])  # (B, T, 32)
    ...
    def forward(self, static_input, seq_input):
        # Ensure seq_input matches static_input device
        assert static_input.device == seq_input.device, f"Device mismatch: {static_input.device} vs {seq_input.device}"
...
#         # Check if the model is wrapped in DataParallel and load accordingly
#         if isinstance(model, nn.DataParallel):
#             model.module.load_state_dict(checkpoint['model_state_dict'])  # unwrap model
#         else:
...
def kfold_train(dataset, k_folds, prev_trained_days, batch_size, model_dir, num_epochs):
    ...
    if start_epoch ==20 and start_fold==4:
        start_epoch=0
        start_fold=0
    ...
    for fold_idx in range(start_fold, k_folds):
        ...
        model_save_path = os.path.join(model_dir, f"unet_model_fold_{fold_idx+1+prev_trained_days}.pt")
        ...
        np.save(os.path.join(model_dir, f"train_val_losses_fold_{fold_idx + 1+ prev_trained_days}.npy") , fold_losses)
        # np.save(f"/usr/workspace/lazin1/anaconda_dane/envs/RAPID/normalized_UNet/val_miou_{fold + 1}.npy" , miou)
        np.save(os.path.join(model_dir, f"val_mse_{fold_idx + 1+ prev_trained_days}.npy"),mse)
        print(f"Model for fold {fold_idx + 1 + prev_trained_days} saved as {model_save_path}.\n")
        start_epoch = 0

def test_unet_on_dataset(device, model, dataset, flood_max = float):
...
#         static_input, precip_tensor, output_geotiff_paths, transforms, crs_list = data[0], data[1], data[2][0],data[3][0] , data[4][0] 
#         print(output_geotiff_paths)
#         with torch.no_grad():
...
#             #         # Ensure shape is (B, H, W) for saving
#             if outputs.dim() == 4:
#                 outputs = outputs.squeeze(1)  # (B, H, W)

#             outputs = outputs.cpu().numpy()  # Convert to NumPy
...
# def test_unet_on_dataset(model, dataloader, device):
    # model.eval()
    # with torch.no_grad():
    #     for static_inputs, precip_inputs, paths, transforms, crs_list in tqdm(dataloader):
    #         static_inputs = static_inputs.to(device)      # (B, 2, H, W)
    #         precip_inputs = precip_inputs.to(device)      # (B, T, H, W)

    #         outputs = model(static_inputs, precip_inputs)  # (B, 1, H, W) or (B, H, W)
...
DEM_mean = np.mean(np.load(DEM_mean_file)).astype(np.float32)
DEM_std = np.mean(np.load(DEM_std_file)).astype(np.float32)
...
flow_mean = np.mean(np.load(flow_mean_file)).astype(np.float32)
flow_std = np.mean(np.load(flow_std_file)).astype(np.float32)
...
H1_prec_mean = np.mean(np.load(H1_prec_mean_file)).astype(np.float32)
H1_prec_std = np.mean(np.load(H1_prec_std_file)).astype(np.float32)
...
# output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_event2_prec_dem_LC_Hourly"
# os.makedirs(output_geotiff_dir, exist_ok=True)

model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_prec_dem_LC_HOURLY_script_normalized_without_LC"
...
if sys.argv[1] == "train":
    ...
    # --- Time setup ---
    sequence_length = 6
    start_time = datetime.strptime("2019022601", "%Y%m%d%H")
    ...
    dataset = FloodDataset(event, dem_dir, lc_dir, precip_dir, flood_dir, tile_csv, all_hours, sequence_length,
                            DEM_mean, DEM_std, H1_prec_mean, H1_prec_std,
                            flood_min=0, flood_max=18, output_geotiff_dir=None, return_metadata=False, mode='train')
        # print(dataset.samples[0])
    # print("dataset line 632")
    ...
    # print(dataset[0])
    kfold_train(dataset, k_folds=4, batch_size=16, prev_trained_days=0,  model_dir=model_dir, num_epochs=20)
...
#     dataset = StaticFloodDataset(all_data)
    ...
    model.load_state_dict(state_dict)
    output_geotiff_dir =f"/p/vast1/lazin1/UNet_Geotiff_output/Sonoma_normalized_without_LC/test"
    os.makedirs(output_geotiff_dir, exist_ok=True)
    ...
    test_unet_on_dataset(device, model, dataset, flood_max=18, )
