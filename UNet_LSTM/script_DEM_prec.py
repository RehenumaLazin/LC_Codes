...
class UNetLSTM(nn.Module):
    def __init__(self, in_static_ch=1, in_seq_ch=1, seq_len=10, hidden_dim=64):  #CHANGED
        super().__init__()
...
model_dir = f"/p/vast1/lazin1/UNet_trains/Sonoma_prec_dem"
...
if sys.argv[1] == "train":
    ...
    # print(dataset[0])
    kfold_train(dataset, k_folds=5, batch_size=16, prev_trained_days=0,  model_dir=model_dir, num_epochs=20)
...
#     dataset = StaticFloodDataset(all_data)
    ...
    model_path = f"{model_dir}/unet_model_fold_11_.pt"
    ...
    end_time = datetime.strptime("2019030105", "%Y%m%d%H")
    all_hours = pd.date_range(start=start_time, end=end_time, freq='h')
    dataset = FloodDataset(event, dem_dir, lc_dir, precip_dir, flood_dir, tile_csv, all_hours, sequence_length,
                        DEM_mean, DEM_std, H1_prec_mean, H1_prec_std,
